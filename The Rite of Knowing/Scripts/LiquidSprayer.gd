...
@export var MAX_LIQUID_PARTICLES = 1000;
...
func create_particle():
	...
	trans.origin += Vector2(randf_range(-.5,.5),randf_range(-.5,1))
	...
	ps.body_set_param(water_col, PhysicsServer2D.BODY_PARAM_GRAVITY_SCALE, 10.0)
	...
	particles.append([water_col, water_particle])

func _physics_process(delta):
	#add particles while less than max amount set and timer < 0
	if spawn_timer < 0 and current_particle_count < MAX_LIQUID_PARTICLES:
		create_particle()
